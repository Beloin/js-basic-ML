module.exports = class KMeans {
    constructor() {
        ...
        this.indice2 = Math.floor(Math.random() * this.eixoX.length);
    ...
    indiceCentroide() {
        ...
        let positivo = fs.readFileSync('positivo.txt', 'utf-8');
        ...
        if (positivo.length > 0) {
            ...
            let negativo = fs.readFileSync('negativo.txt', 'utf-8');
    ...
    inicializaIndices(matriz = []) {
        ...
        const i2 = Math.floor(Math.random() * this.eixoX.length);
        const sorteio = [i1, i2];
        let igual = 0;
    ...
    feedback() {
        ...
        if (positivo.length <= 0) {
            ...
            readLine.question('Feedback positivo ou negativo? ', feed => {
                ...
                if (feed.toLowerCase().trim() == 'positivo')
                    nome_arquivo = 'positivo.txt';

                let antigo = fs.readFileSync(nome_arquivo, 'utf-8');
                if (antigo == undefined) antigo = '';
        ...
        for (let i = 0; i < this.eixoX.length; i++) {
            for (let j = 0; j < this.grupos; j++) {
                distancias[j] = Math.sqrt(
                    Math.pow(this.eixoX[i] - this.centroidesX[j], 2) +
    ...
    predict() {
        ...
        if (this.grupos > 1 && this.grupos < this.eixoX.length) {
            if (this.eixoX.length > 2) {
                ...
                const matriz = this.retornaElementosGrupo(
                    this.grupoID_Anterior
                );
				console.log(matriz);
				this.feedback();
